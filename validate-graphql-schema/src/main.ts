...
async function run(): Promise<void> {
  ...
  const schema_path = core.getInput('schema', {required: true}) || 'schema.graphql'
  ...
  try {
    const schema = async() => await loadSchema(schema_path, {
      loaders: [new GraphQLFileLoader()]
    })
  
    // load from endpoint
    const endpoint = async() => await loadSchema(endpoint_path, {
